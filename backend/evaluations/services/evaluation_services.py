...
@transaction.atomic
def evaluation_update(*, evaluation_id, data: dict, actor: User) -> Evaluation:
    try:
        ev = Evaluation.objects.select_related("student").get(id=evaluation_id)
    except Evaluation.DoesNotExist:
        raise ValidationError("الاختبار غير موجود.")

    _check_teacher_of_student(actor=actor, student=ev.student)

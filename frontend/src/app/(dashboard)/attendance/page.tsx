    ...
    const buildDrafts = async () => {
      ...
      (students ?? []).forEach((s) => {
        ...
        map.set(s.id, {
          ...
          from_ayah: existing?.from_ayah ?? "",
          to_ayah: existing?.to_ayah ?? "",
          from_page: existing?.from_page ?? "",
          to_page: existing?.to_page ?? "",
  ...
  const saveAll = async () => {
    ...
    for (const d of dirtyDrafts) {
      ...
      const payload = {
        ...
        from_ayah: d.from_ayah === "" ? null : Number(d.from_ayah),
        to_ayah: d.to_ayah === "" ? null : Number(d.to_ayah),
        from_page: d.from_page === "" ? null : Number(d.from_page),
        to_page: d.to_page === "" ? null : Number(d.to_page),
